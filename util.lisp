...
;;
;; The spec file contains mostly useless crap.  For now:
;; As we load it, write a file containing a list of useful line numbers
;; Afterwards, we manually trim-spec using the original specfile and this
;; trimfile, writing only the lines we need.
(defun trim-spec (specdir specfile)
  (with-open-file (hin (merge-pathnames specfile specdir))
    (with-open-file (hout (merge-pathnames "proposed.spec" specdir)
			  :direction :output :if-exists :supersede)
      (let ((lines
	     (with-open-file (htrim
		  (merge-pathnames "trim.sexp" specdir))
	       (read htrim)))
	    (current-line 0)
	    (expression nil))
	(loop for target-line in lines do
	     (loop while (< current-line target-line) do
		  (setf expression (read-line hin))
		  (incf current-line))
	     (print expression)
